...
def run_pipeline(source: str,
    ...
    if carry:
        ...
        if plan:
            ...
            # Если план вернул свой final_level как ориентир — не перетираем start_level.
            # start_level берём именно из клетки робота (см. выше).
        success = True
    elif interactive:
...
def main():
    ...
    ap.add_argument("--speed", action="store_true",
                    help="При равной длине маршрута выбирать путь с меньшим "
                         "числом поворотов (меньше торможений)")
